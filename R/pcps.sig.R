pcps.sig<-function(comm, dist.spp, envir, method = "bray", squareroot = TRUE, formula, family = gaussian, runs = 999, AsFactors = NULL){
	dis<-dist.spp
	...
	for(k in 1:runs){
		...
		mod_null<-glm(formula,data=data_null,family=family)
	...
	p_taxa<-(sum(ifelse(res_F_null>=f_obs,1,0))+1)/(runs+1)
	p_site<-(sum(ifelse(res_F_null_2>=f_obs,1,0))+1)/(runs+1)
return(list(model=mod_obs, Envir_class=envir_class, formula=formula, f.obs=f_obs, p.site.shuffle=p_site, p.taxa.shuffle=p_taxa))
}